...
| Category | Tools |
|---|---|
| Discovery | `list_currencies`, `get_markets`, `search_coins`, `get_coin_metadata`, `get_coin_history`, `doctor` |
...
| Account / funds | `approval_status`, `build_approve`, `build_deposit`, `build_transfer`, `send_tx`, `send_transfer` |
| Withdraw | `withdraw_request`, `withdraw_build`, `withdraw_send` |
| Debugging / helpers | `verify_signature`, `permit_metadata` |
| Maker / order book | `place_order`, `cancel_order`, `cancel_all_orders`, `place_vl_batch`, `cancel_vl_batch`, `get_order`, `list_orders`, `get_fills`, `get_fills_for_order` |

`sera.approval_status` checks a token's allowance specifically for the live Vault address. If it is short, it returns the complete safe next call to `sera.build_approve`; it does not create, sign, or send a transaction.
